...
### 笔记上下滚动（无限笔记）
- 写满一屏后**不必清屏**：屏幕右缘有竖向胶囊滚动条，点击上/下按钮即可整体平移已有墨迹，继续向下书写
- 中间的位置指示条表示当前浏览位置相对全部内容深度的比例
...
在**右键菜单 → 快捷键**中可查看全部快捷键、逐个修改或恢复默认，保存后即时生效（存于 Settings.json）。
有墨迹时触发"重启画板/退出"会先弹出确认，防止误触丢失板书。

### 悬浮工具条
- 启动时默认收起为一个**笑脸把手**：单击展开工具条，按住可拖动到任意位置
- 右键笑脸或工具条弹出菜单：展开/收起、回到默认位置、设置、重启画板、退出
